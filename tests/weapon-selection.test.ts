...
ok('bot pick does NOT clobber the human weapon', human.getWeaponIndex() === CTRL, `human=${human.getWeaponIndex()}`);
...
gc.beginTurn();
ok('human weapon restored on their turn', gc.getCurrentWeaponIndex() === CTRL, `got ${gc.getCurrentWeaponIndex()} (bot had ${bot.getWeaponIndex()})`);
ok('restored weapon matches the control weapon type', getWeapon(gc.getCurrentWeaponIndex()).getType() === WEAPON_DATABASE[CTRL].type);

// 7. Tanks hold independent weapons at the same time.
ok('players keep independent weapon state', human.getWeaponIndex() === CTRL && typeof bot.getWeaponIndex() === 'number');
