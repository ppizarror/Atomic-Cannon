...
// 1. Aim converges on flat ground with no wind — the solved arc lands ON the target.
{
  ...
  ok('flat-ground solution is a near-hit', aim.dist < 20, `dist=${aim.dist.toFixed(1)} a=${aim.angleDeg} p=${aim.power}`);
...
// 2. Aim works to the LEFT (angles past 90°) too.
{
  ...
  ok('leftward solution is a near-hit', aim.dist < 20, `dist=${aim.dist.toFixed(1)} a=${aim.angleDeg}`);
  ok('leftward aim points left (90..180)', aim.angleDeg > 90 && aim.angleDeg < 180, `a=${aim.angleDeg}`);
}
...
// 4. Aim arcs OVER a hill sitting between the tank and the target.
{
  ...
  ok('clears an intervening hill', aim.dist < 30, `dist=${aim.dist.toFixed(1)} a=${aim.angleDeg} p=${aim.power}`);
}
...
// 5. Difficulty model: P(aim) grows with level; angle scatter shrinks to 0 at max.
{
  ...
  ok('P(aim) is 0.75 at level 5', Math.abs(aimProbability(5) - 0.75) < 1e-9, `${aimProbability(5)}`);
  ...
  ok('level 0 scatter reaches 8°', Math.abs(Math.abs(angleError(AI_LEVEL_MIN, () => 1)) - 8) < 1e-6, `${angleError(AI_LEVEL_MIN, () => 1)}`);
  // Level 5 scatter is in [1.0, 4.0]°, strictly less than level 0's.
  ok('scatter shrinks with level', Math.abs(angleError(5, () => 1)) < Math.abs(angleError(0, () => 1)));
