...
interface WX {
  m_layers: Layer[]; m_w: number; m_h: number; m_margin: number;
...
//    (independent of intensity): dust (×3) > snow (×1.5) > rain = hail (×1).
{
  const wx = make();
  wx.configure([
    { type: 'snow', intensity: 100 }, { type: 'rain', intensity: 100 },
    { type: 'hail', intensity: 100 }, { type: 'dust', intensity: 100 },
  ]);
  const rain = countOf(wx, 'rain'), snow = countOf(wx, 'snow'), dust = countOf(wx, 'dust'), hail = countOf(wx, 'hail');
  ok('all four bands present at intensity 100', wx.m_layers.length === 4, `layers=${wx.m_layers.length}`);
  ...
  ok('dust ≈ 3× rain (thickest field)', Math.abs(dust - rain * 3) <= 2, `dust=${dust} rain=${rain}`);
}
...
{
  const wx = make();
  wx.configure([{ type: 'fog', intensity: 100 }, { type: 'rain', intensity: 0 }, { type: 'rain', intensity: 100 }]);
...
{
  const wx = make();
  wx.configure([{ type: 'dust', intensity: 100 }, { type: 'rain', intensity: 100 }, { type: 'snow', intensity: 100 }, { type: 'hail', intensity: 100 }]);
  wx.update(1 / 60, new Vec2(2, 0));
  const ctx = mockCtx();
  let threw = false;
  try { wx.draw(ctx); } catch { threw = true; }
