...
// 1. Buy deducts cost and adds stock; guarded by affordability.
{
  ...
  ok('buy deducts cost + adds a round', e.buy(ROCKET) && e.getCredits() === 1000 - rocketCost && e.getOwned(ROCKET) === 1, `cr=${e.getCredits()} owned=${e.getOwned(ROCKET)}`);
  const e2 = new CEconomy(100, []);
  ok('cannot buy what you cannot afford', !e2.canBuy(NUKE) && !e2.buy(NUKE) && e2.getOwned(NUKE) === 0);
}
...
// 2. Sell refunds half (rounded) and removes stock.
{
  ...
  e2.buy(NUKE);
  ok('sell refunds 50% and decrements', e2.sell(NUKE) && e2.getOwned(NUKE) === 0 && e2.getCredits() === Math.round(nukeCost * 0.5), `cr=${e2.getCredits()}`);
...
// 3. Unlimited staples: never bought/sold, always fireable.
{
  ...
  ok('unlimited always has stock + consume', e.hasStock(ROCKET) && e.consume(ROCKET) && e.getOwned(ROCKET) === UNLIMITED);
}

// 4. Consume depletes finite stock, blocks when empty.
{
  const e = new CEconomy(rocketCost * 2, []);
  e.buy(ROCKET); e.buy(ROCKET);
