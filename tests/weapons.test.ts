...
// ---- 1. Airburst detonates at apex (mid-air, before hitting ground) ----------
{
  ...
  ok(
    'Airburst detonates at apex, above ground',
    action === 'detonate' && y < 300,
    `action=${action} y=${y.toFixed(0)}`,
  );
  ok(
    'Airburst apex is the highest point reached',
    Math.abs(y - apexY) < 20,
    `y=${y.toFixed(0)} apex=${apexY.toFixed(0)}`,
  );
}
...
// ---- 2. Cluster: cluNum children at gen+1 and 0.5x power ----------------------
{
  ...
  ok(
    ...
    world.spawned.every(s => s.getGeneration() === 1),
  );
  ok(
    'Children launch at 0.5x power',
    world.spawned.every(s => s.getPower() === 50),
    `pow=${world.spawned[0]?.getPower()}`,
  );
...
// ---- 3. Cluster recursion depth honours cluRecurse ---------------------------
{
  ...
  ok(
    'Cluster stops recursing past cluRecurse',
    world.spawned.length === 0,
    `got ${world.spawned.length}`,
  );
}
...
// ---- 5. NUKE creates a radiation zone ----------------------------------------
{
  ...
  weaponDetonate(shot, w, world);
  ok(
    'NUKE creates a radiation zone',
    land.getRadiationZones().length === 1,
    `zones=${land.getRadiationZones().length}`,
  );
}
...
// ---- 6. Roller converts to a horizontal surface roll -------------------------
{
  ...
  ok(
    'Roller rolls horizontally (vy≈0)',
    Math.abs(shot.getVelocity().y) < 1 && action === 'continue',
    `action=${action}`,
  );
...
// ---- 7. Beam damages full (no falloff) & no crater ---------------------------
{
  ...
  weaponDetonate(shot, w, world);
  ok(
    'Beam leaves no crater',
    land.getHeightAt(400) === before,
    `before=${before} after=${land.getHeightAt(400)}`,
  );
}
