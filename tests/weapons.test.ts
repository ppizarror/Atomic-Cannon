...
function flatLand(surface = 300): CLand {
  ...
  const h = new Int16Array(800); h.fill(surface);
...
class MockWorld implements ShotWorld {
  ...
  tanks: any[] = [];
  spawned: CShot[] = [];
  mines = 0; sentries = 0; markers = 0; blasts: number[] = [];
  constructor(land: CLand) { this.land = land; }
  spawnShot(s: CShot) { this.spawned.push(s); }
  explode() {} shake() {} ripple() {}
  applyBlast(_p: Vec2, _r: number, dmg: number) { this.blasts.push(dmg); }
