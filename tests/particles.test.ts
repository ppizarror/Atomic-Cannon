...
// 4c. Higher shot speed (∝ power) lays down more trail smoke per frame.
{
  ...
  ok(
    'faster shot emits more trail smoke',
    hi.count() > lo.count(),
    `lo=${lo.count()} hi=${hi.count()}`,
  );
}
...
// 4d. A named preset drives explosion colour (eGreen → green fireball particles).
{
  ...
  const parts = (ps as unknown as {m_particles: {r: number; g: number; b: number; kind: string}[]})
    .m_particles;
  ...
  ok(
    ...
    flares.length > 0 && greenish > flares.length * 0.5,
...
// 9b. Normal-blend kinds (disc/smoke) paint under 'source-over'.
{
  ...
  ok(
    ...
    sctx._calls.every(c => c.op === 'source-over'),
    sctx._calls.map(c => c.op).join(','),
  );
}
...
//     and globalAlpha back to 1 (no leaked alpha from a mid-blit).
{
  ...
  ps.draw(ctx);
  ok(
    'full scene restores composite op',
    ctx.globalCompositeOperation === 'multiply',
    ctx.globalCompositeOperation,
  );
...
//     rather than only filling — the sprite fast-path stays wired up.
{
  ...
  ok(
    ...
    ctx._calls.some(c => c.m === 'drawImage'),
    ctx._calls.map(c => c.m).join(','),
  );
}
